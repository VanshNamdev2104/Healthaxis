...
import { useMedicine } from '../../health/hooks/useMedicine';
import MedicineUserCard from '../components/MedicineUserCard';
import { gsap } from 'gsap';
import { Search, Sparkles, Pill, Activity } from 'lucide-react';
...
const Medicine_user = () => {
  ...
  useEffect(() => {
    if (!loading && medicines?.medicines?.length > 0) {
      gsap.fromTo('.user-medicine-card', 
        { y: 50, opacity: 0 }, 
    ...
    <div className="min-h-screen bg-[#fafaf5] text-[#1a1c19] font-sans selection:bg-[#553722] selection:text-white overflow-x-hidden relative pb-24">
      ...
      <div className="fixed inset-0 pointer-events-none z-0">
        {[...Array(6)].map((_, i) => (
          ...
          >
            {i % 2 === 0 ? (
               <Pill className="text-[#553722]" size={Math.random() * 100 + 50} />
            ) : (
      ...
      >
        <div ref={titleRef} className="space-y-6">
          <div className="inline-flex items-center gap-3 px-6 py-2 rounded-full bg-[#553722]/5 border border-[#553722]/10">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.4em] text-[#553722]">Pharmacopoeia</span>
          </div>
          
          <h1 className="text-6xl md:text-8xl font-serif font-bold tracking-tighter leading-none text-[#1a1c19]">
            Scientific <br />
            <span className="text-transparent bg-linear-to-r from-[#553722] via-[#a68966] to-[#553722] bg-clip-text">Restoration.</span>
          </h1>
          
          <p className="text-lg md:text-xl text-[#50453e] font-medium max-w-2xl mx-auto leading-relaxed">
            Discover our comprehensive registry of medications, dosages, and therapeutic insights. 
            Empowering your path to recovery with precision.
          </p>
        </div>
      ...
      <main className="relative z-10 px-8 max-w-7xl mx-auto">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-40">
            ...
            <p className="mt-6 text-[#553722] font-black uppercase tracking-widest text-[10px]">Accessing Registry...</p>
          </div>
        ) : (
          <div 
            ref={gridRef}
      ...
      <footer className="mt-32 text-center relative z-10">
        <div className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-[#553722]/40">
          <Activity size={12} />
          <span>Healthaxis AI • Pharmaceutical Precision</span>
        </div>
      </footer>
    </div>
  );
